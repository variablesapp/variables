public class Variables.MainViewModel : GLib.Object {
    ...
    public Variables.TemplateViewModel template_view_model { get; construct; }
    ...
    construct {
        ...
        this.template_view_model = new Variables.TemplateViewModel ();
        ...
        this.templates_view_model.template_selection_changed.connect ((template) => {
            ...
            this.try_update_output (template);
        });

        if (this.templates_view_model.selection_model.selected_item != null) {
            this.try_update_output ((Variables.Template) this.templates_view_model.selection_model.selected_item);
        }

        this.variables_view_model.variable_property_changed.connect (() => {
            this.template_view_model.request_output_update ();
        });
        ...
        container.bind_instance (typeof (Variables.TemplateViewModel), this.template_view_model);
    }

    private void try_update_output (Variables.Template template) {
        this._selected_template = template;
        this.variables_view_model.load_variables (this._selected_template);
        this.template_view_model.load_template (this._selected_template);
    }
